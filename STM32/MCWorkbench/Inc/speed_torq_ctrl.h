...
typedef struct {
  MC_ControlMode_t Mode; /*!< Modality of STC. It can be one of these two
                            settings: MCM_TORQUE_MODE to enable the Torque mode
                            or MCM_SPEED_MODE to enable the Speed mode. */
  int16_t TargetFinal;   /*!< Backup of #hTargetFinal to be applied in the last
                            step. */
  int32_t SpeedRefUnitExt; /*!< Current mechanical rotor speed reference
                              expressed in [SPEED_UNIT](measurement_units.md)
                              multiplied by 65536.*/
  int32_t TorqueRef; /*!< Current motor torque reference. This value represents
                        actually the Iq current expressed in digit multiplied by
                        65536. */
  uint32_t
  ...
  uint16_t STCFrequencyHz; /*!< Frequency on which the user updates the torque
                              reference calling #STC_CalcTorqueReference method
                              expressed in Hz */
  uint16_t MaxAppPositiveMecSpeedUnit; /*!< Application maximum positive value
                                          of the rotor mechanical speed.
  ...
  uint16_t MinAppPositiveMecSpeedUnit; /*!< Application minimum positive value
                                          of the rotor mechanical speed.
                                            Expressed in the unit defined by
                                          [SPEED_UNIT](measurement_units.md). */
  int16_t MaxAppNegativeMecSpeedUnit; /*!< Application maximum negative value of
                                         the rotor mechanical speed. Expressed
                                         in the unit defined by
                                         [SPEED_UNIT](measurement_units.md). */
  int16_t MinAppNegativeMecSpeedUnit; /*!< Application minimum negative value of
                                         the rotor mechanical speed. Expressed
                                         in the unit defined by
                                         [SPEED_UNIT](measurement_units.md). */
  uint16_t MaxPositiveTorque; /*!< Maximum positive value of motor torque. This
                                 value represents actually the maximum Iq
                                 current expressed in digit. */
  int16_t MinNegativeTorque;  /*!< Minimum negative value of motor torque. This
  ...
  int16_t MecSpeedRefUnitDefault; /*!< Default mechanical rotor speed reference
                                     expressed in the unit defined by
                                       [SPEED_UNIT](measurement_units.md). */
  int16_t TorqueRefDefault; /*!< Default motor torque reference. This value
                               represents actually the Iq current reference
                               expressed in digit. */
 ...
 */
void STC_SetControlMode(SpeednTorqCtrl_Handle_t* pHandle,
                        MC_ControlMode_t bMode);
...
/* Select the sensor as primary sensor if the angle error is below the
 * authorized margin */
bool STC_SensorSet(SpeednTorqCtrl_Handle_t* pHandle, int16_t error,
                   uint16_t margin, SpeednPosFdbk_Handle_t* sensor);

/**
 * @brief  Gets the modality of the speed and torque controller
 *           @ref SpeednTorqCtrl_Handle_t::Mode "Mode".
 * @param  pHandle: handler of the current instance of the SpeednTorqCtrl
 * component.
 * @retval MC_ControlMode_t  modality of STC. It can be one of
 *         these two values: MCM_TORQUE_MODE or MCM_SPEED_MODE.
 *
 * - Called by @ref SpeedRegulatorPotentiometer Speed potentiometer component to
 * manage new speed reference.
 */
static inline MC_ControlMode_t STC_GetControlMode(
...
/**
 * @brief  Interrupts the execution of any previous ramp command in particular
 * by clearing the number of steps remaining to complete the ramp
...
/**
 * @brief  Checks if the settled speed or torque ramp has been completed by
 * checking zero value of
 *           @ref SpeednTorqCtrl_Handle_t::RampRemainingStep
 * "RampRemainingStep".
 * @param  pHandle: handler of the current instance of the SpeednTorqCtrl
 * component.
 ...
 *         @ref SpeednTorqCtrl_Handle_t::MaxPositiveTorque "MaxPositiveTorque"
 * and
 *         @ref SpeednTorqCtrl_Handle_t::MinNegativeTorque "MinNegativeTorque".
 * @param  pHandle: handler of the current instance of the SpeednTorqCtrl
 * component.
 * @param  hNominalCurrent: represents actually the maximum Iq current expressed
 * in digit.
 * @retval none
 *
 * - Not used into current implementation.
 */
static inline void STC_SetNominalCurrent(SpeednTorqCtrl_Handle_t* pHandle,
 ...
 */
static inline SpeednPosFdbk_Handle_t* STC_GetSpeedSensor(
    SpeednTorqCtrl_Handle_t* pHandle) {
#ifdef NULL_PTR_CHECK_SPD_TRQ_CTL
 ...
 * - Called as soon as component parameters are required by MC FW.
 */
static inline SpeednPosFdbk_Handle_t* STC_GetAuxSensor(
    SpeednTorqCtrl_Handle_t* pHandle) {
#ifdef NULL_PTR_CHECK_SPD_TRQ_CTL
...
/************************ (C) COPYRIGHT 2026 STMicroelectronics *****END OF
 * FILE****/
