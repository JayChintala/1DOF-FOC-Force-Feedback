...
typedef void (*ASPEP_config_transmission_cb_t)(void* pASPEP_Handle,
                                               void* txbuffer, uint16_t length);
typedef void (*ASPEP_config_reception_cb_t)(void* pASPEP_Handle, void* rxbuffer,
                                            uint16_t length);
...
typedef struct {
  ...
  MCTL_Buff_t
      asyncBufferA; /*!< First buffer used for asynchronous communication */
  MCTL_Buff_t
      asyncBufferB; /*!< Second buffer used for asynchronous communication */
  MCTL_Buff_t* lastRequestedAsyncBuff; /*!< Last buffer requested for
                                          asynchronous communication */
  MCTL_Buff_t* asyncNextBuffer; /*!< Next buffer that will be transmitted by
                                   asynchronous communication */
  void* lockBuffer; /*!< Buffer locked to avoid erasing data not yet transmitted
                     */
  ASPEP_hwinit_cb_t
      fASPEP_HWInit; /*!< Pointer to the initialization function */
  ASPEP_hwsync_cb_t fASPEP_HWSync; /*!< Pointer to the starting function */
  ASPEP_config_reception_cb_t
      fASPEP_cfg_recept; /*!< Pointer to the receiving packet function */
  ASPEP_config_transmission_cb_t
      fASPEP_cfg_trans;   /*!< Pointer to the sending packet function */
  uint16_t rxLengthASPEP; /*!< Length of the received data packet used by ASPEP
                             : payload, header or both */
  uint16_t maxRXPayload;  /*!< Maximum payload size the performer can process */
  uint8_t
      syncPacketCount; /*!< Reset at startup only, this counter is incremented
                          at each valid data packet received from controller */
  bool NewPacketAvailable; /*!< Boolean stating if performer is ready to receive
                              a new packet or not */
  uint8_t badPacketFlag; /*!< Contains the error code in case of ASPEP decoding
                            issue */
  uint8_t liid;
  ASPEP_sm_type ASPEP_State; /*!< ASPEP State of the communication between
                                performer and controller */
  ASPEP_TL_sm_type
      ASPEP_TL_State; /*!< Transport Layer state of the communication between
                         performer and controller */
  ASPEP_packetType rxPacketType;       /*!< Type of the received packet */
  ASPEP_Capabilities_def Capabilities; /*!< Minimum between Controller and
...
uint8_t ASPEP_sendPacket(MCTL_Handle_t* pHandle, void* txBuffer,
